...
define create_push_catalog_image
	...
	if [[ "$$(echo $(4) | tr [:upper:] [:lower:])" == "true" ]]; then \
		deployed_hash=$$(curl -s 'https://gitlab.cee.redhat.com/$(5)/raw/master/$(6)' | $(CONTAINER_ENGINE) run --rm -i quay.io/app-sre/yq -r '.services[]|select(.name="$(OPERATOR_NAME)").hash') ;\
		delete=false ;\
