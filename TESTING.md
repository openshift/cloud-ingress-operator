...
Testing guidelines for the Cloud Ingress Operator.
...
go test ./controllers/publishingstrategy/
...
**Minimal validation:**
```bash
# After changing controllers/publishingstrategy/
go test ./controllers/publishingstrategy/
```
...
```go
It("should configure publishing strategy", func() {
    // Create PublishingStrategy resource
    pubStrategy := &v1alpha1.PublishingStrategy{...}
    Expect(k8sClient.Create(ctx, pubStrategy)).To(Succeed())
