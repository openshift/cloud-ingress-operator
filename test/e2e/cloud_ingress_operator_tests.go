		...
		defer func() {
			...
			for attempt := 1; attempt <= maxAttempts; attempt++ {
				...
				if getErr := k8s.Get(ctx, apiSchemeResourceName, config.OperatorNamespace, &fresh); getErr != nil {
					log.Printf("Warning: APIScheme revert skipped (fetch failed, attempt %d/%d)", attempt, maxAttempts)
					return
				}
				...
				if updateErr := k8s.Update(ctx, &fresh); updateErr == nil {
					...
					log.Printf("Warning: APIScheme revert skipped (update failed, attempt %d/%d)", attempt, maxAttempts)
					return
				}
				if attempt < maxAttempts {
					log.Printf("Conflict reverting APIScheme, retrying (attempt %d/%d)", attempt, maxAttempts)
				}
			}
			log.Printf("Warning: APIScheme revert failed after %d conflict retries", maxAttempts)
		}()
