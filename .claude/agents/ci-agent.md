...
- `cloud-ingress-operator-pull-request.yaml`: PR validation
- `cloud-ingress-operator-push.yaml`: Main branch builds
- `cloud-ingress-operator-pko-pull-request.yaml`: PKO validation
- `cloud-ingress-operator-pko-push.yaml`: PKO deployment
...
# tkn pipeline validate -f .tekton/cloud-ingress-operator-pull-request.yaml
...
```text
CI Status: FAILING
Pipeline: cloud-ingress-operator-pull-request
Stage: golangci-lint
Error: Exit code 1

Local Reproduction:
  make go-check
  # Output shows 3 linter errors in controllers/publishingstrategy/publishingstrategy.go
