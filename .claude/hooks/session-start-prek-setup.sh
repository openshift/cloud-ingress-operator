...
# Use git rev-parse --git-path to get correct hooks directory (works in worktrees)
HOOKS_DIR=$(git rev-parse --git-path hooks)
if [[ ! -f "$HOOKS_DIR/pre-commit" ]] || ! grep -q "prek" "$HOOKS_DIR/pre-commit" 2>/dev/null; then
