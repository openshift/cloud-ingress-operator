...
func (c *Client) ensureDNSForService(kclient client.Client, svc *corev1.Service, dnsName string) error {
	...
	//this will never happen, but here for safety check
	if fr == nil {
		return cioerrors.ForwardingRuleNotFound("Unknown return getting forwarding rule for IP " + rhapiLbIP)
	}
...
// Returns GCP forwarding rule for given IP or error if not found
